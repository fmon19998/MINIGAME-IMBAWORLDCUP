...
function rewardInfo(saved, missed = 0) {
  const safeSaved = Number(saved || 0);
  const safeMissed = Number(missed || 0);
  if (safeSaved > 60 && safeMissed <= 60) return { reward: 20000, goalText: 'Syarat klaim terpenuhi: tepis lebih dari 60 dan kebobolan maksimal 60.' };
  if (safeMissed > 60) return { reward: 0, goalText: 'Gagal klaim: bola kebobolan sudah lebih dari 60.' };
  const left = Math.max(0, 61 - safeSaved);
  return { reward: 0, goalText: `Kurang ${left} tepis lagi. Syarat klaim: tepis 61+ dan kebobolan maksimal 60.` };
}

function updateProgress(saved, remaining, stats = {}) {
  const safeSaved = Number(saved || 0);
  const percent = Math.max(0, Math.min(100, (safeSaved / 60) * 100));
  const safeMissed = Number(stats.missed || 0);
  const info = rewardInfo(safeSaved, safeMissed);
...
function renderResult(result) {
  ...
  $('#resultNote').textContent = result.reward > 0
    ...
    : 'Syarat klaim belum terpenuhi: tepis harus lebih dari 60 dan bola kebobolan tidak boleh lebih dari 60.';
