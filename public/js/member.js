...
function updateProgress(saved, remaining, stats = {}) {
  ...
  if (modeLabel) {
    if (timeText <= 12 || safeSaved >= 45) modeLabel.textContent = 'ULTRA PANAS';
    else if (timeText <= 25 || safeSaved >= 25) modeLabel.textContent = 'SUPER SUSAH';
    else modeLabel.textContent = 'MODE SULIT';
  }
  if (dangerText) {
    ...
    else dangerText.textContent = 'BOLA DATANG DARI SEGALA ARAH!';
  }
}
