  ...
  class ImbaKeeperGame {
    constructor(options) {
      ...
      this.keeper = {
        x: this.width / 2,
        y: this.height - 82,
        w: clamp((this.isTouchDevice ? 112 : 92) + Number(this.gameConfig.keeperWidthBonus || 0), 68, 160),
        h: 28,
        vx: 0
      };
    ...
    difficulty(elapsed) {
      ...
      let base;
      if (elapsed > 48) base = { spawn: 250 + touchSpawnBonus, speedMin: 820 - touchSpeedCut, speedMax: 1120 - touchSpeedCut, curve: 175, size: [11, 16] };
      else if (elapsed > 36) base = { spawn: 300 + touchSpawnBonus, speedMin: 760 - touchSpeedCut, speedMax: 1040 - touchSpeedCut, curve: 150, size: [12, 17] };
      else if (elapsed > 22) base = { spawn: 360 + touchSpawnBonus, speedMin: 700 - touchSpeedCut, speedMax: 950 - touchSpeedCut, curve: 130, size: [12, 18] };
      else if (elapsed > 10) base = { spawn: 420 + touchSpawnBonus, speedMin: 650 - touchSpeedCut, speedMax: 870 - touchSpeedCut, curve: 100, size: [13, 18] };
      else base = { spawn: 500 + touchSpawnBonus, speedMin: 590 - touchSpeedCut, speedMax: 800 - touchSpeedCut, curve: 80, size: [14, 19] };

      const cfg = this.gameConfig;
      const ballSizeBonus = Number(cfg.ballSizeBonus || 0);
      return {
        spawn: clamp(base.spawn * Number(cfg.spawnMultiplier || 1), 170, 850),
        speedMin: clamp(base.speedMin * Number(cfg.speedMultiplier || 1), 360, 1450),
        speedMax: clamp(base.speedMax * Number(cfg.speedMultiplier || 1), 460, 1600),
        curve: clamp(base.curve * Number(cfg.curveMultiplier || 1), 25, 320),
        size: [
          clamp(base.size[0] + ballSizeBonus, 8, 26),
          clamp(base.size[1] + ballSizeBonus, 10, 30)
        ],
        doubleSpawnChance: clamp(Number(cfg.doubleSpawnChance || 0.62), 0, 0.95)
      };
    }
    ...
    updateKeeper(dt) {
      ...
      this.stamina = clamp(this.stamina - effort * 8.5 * Number(this.gameConfig.staminaDrainMultiplier || 1) * dt + 2.1 * dt, 0, 100);
    }
    ...
    updateBalls(dt, elapsed) {
      ...
      if (this.spawnTimer >= diff.spawn) {
        ...
        if (elapsed > 42 && Math.random() < diff.doubleSpawnChance) this.spawnBall(elapsed);
      }
      ...
      for (const ball of this.balls) {
        ...
        if (!ball.saved && !ball.missed && withinKeeperY && withinKeeperX) {
          ...
          this.score += Math.round((100 + (this.combo * 35)) * Number(this.gameConfig.scoreMultiplier || 1));
