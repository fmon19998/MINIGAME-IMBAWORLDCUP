...
function renderVouchers() {
  ...
  `).join('') || '<tr><td colspan="8">Belum ada data voucher.</td></tr>';
}
...
function renderResults() {
  $('#resultTable').innerHTML = dashboardData.results.map(r => `
    <tr>
      ...
      <td>${r.missed}</td>
      <td>${r.duration}s</td>
      <td>${r.status === 'SUPER_WIN' ? 'MENANG 20RB' : r.status === 'WIN' ? 'MENANG 15RB' : 'KALAH'}</td>
      <td><b>${rupiah(r.reward)}</b></td>
    </tr>
  `).join('') || '<tr><td colspan="8">Belum ada hasil game.</td></tr>';
}
